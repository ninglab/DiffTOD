...
def main():
    ...
    parser.add_argument("--file_save_name", default="cb_conversation_buyer.json", type=str)
    ...
    parser.add_argument("--role", default="buyer", type=str, help="the role the planner plays in the bargain")
    ...
    turns_list = []
    success_list = []
    sell_to_list_ratio_buyer_side_list = []
    for idx, dialog in enumerate(dialogue_corpus):
        dialog_history = ["buyer: Hi, how much is the %s?" % dialog['item_name'], "seller: Hi, this is a good %s and its price is %s." % (dialog['item_name'], dialog['seller_price'])]
        dialog_strategy = ["inquire"]
        for turn in range(args.max_conv_turn):
            ...
            for idx, utterance in enumerate(dialog_history):
                if utterance[:5].lower()=="buyer":
                    first_turn +=  "(strategy: %s) system: "%dialog_strategy[idx//2] + utterance[7:] + "\n"
            ...
            while num_simulations<args.num_mcts_sims:
                ...
                if selected_children is not None:
                    ...
                    guidance = current_node.dialogue_history_so_far + "(strategy: %s) system"%selected_action
                    ...
                    reward = deals
                    #optional, enrouge deal acceptance after long bargains
                    if selected_action in ["agree", "affirm"] and turn>2: reward += 0.5
            ...
            #buyer utterance
            messages = [{"role":"user", "content":"Now enter the role-playing mode. In the following conversation, you will act as a buyer negotiating to purchase the %s for %s. Product description: %s\nRespond with a short, succinct and persuasive sentence aimed at securing the best possible deal. %s Now start the game. " % (dialog['item_name'], dialog['buyer_price'], dialog['buyer_item_description'], CBAct[action])+"\n".join(dialog_history)}]
            ...
            messages = [{"role":"user", "content":"Now enter the role-playing mode. In the following conversation, you will act as a seller negotiating to sell the %s for %s. Product description: %s\nRespond with a short, succinct and persuasive sentence aimed at securing the best possible deal. Now start the game. " % (dialog['item_name'], dialog['seller_price'], dialog['seller_item_description'])+"\n".join(dialog_history)}]
            response = messagefunction(messages).strip()
        ...
        if sell_price:
            sell_to_list_ratio_buyer_side = (float(dialog["sell_price"])-float(dialog["seller_price"]))/(float(dialog["buyer_price"])-float(dialog["seller_price"]))
            sell_to_list_ratio_buyer_side = max(0, min(sell_to_list_ratio_buyer_side, 1))
            sell_to_list_ratio_buyer_side_list.append(sell_to_list_ratio_buyer_side)
        else:
            sell_to_list_ratio_buyer_side_list.append(0)
    ...
    file_to_save.close()
    print("Avg Turn:%.6f. Success rate:%.6f%%. sell_to_list_ratio_buyer_side:.6f%%."%(np.mean(turns_list),np.mean(success_list)*100, np.mean(sell_to_list_ratio_buyer_side_list)*100))
